...
public class FlavorDao {
	...
	private final String GET_FLAVOR_BY_RECIPE_ID_QUERY = "SELECT * FROM flavors f INNER JOIN recipe_flavor rf ON f.id = rf.flavor_id WHERE recipe_id = ?";
	private final String DELETE_FLAVOR_BY_RECIPE_ID_QUERY = "DELETE FROM flavors WHERE id = ?";
	private final String CREATE_NEW_FLAVOR_QUERY = "INSERT INTO flavors(id, name) values(?,?)";
	private final String UPDATE_FLAVOR_BY_NAME_QUERY = "UPDATE flavors SET name = ? WHERE name LIKE ?";
	private final String SEARCH_BY_ONE_FLAVOR_RETURN_RECIPE_LIST_QUERY = "SELECT r.name FROM recipes r INNER JOIN recipe_flavor rf "
			+ "ON r.id = rf.recipe_id INNER JOIN flavors f ON rf.flavor_id = f.id WHERE f.name = ?"; 
	private final String ADD_FLAVOR_TO_DB_QUERY = "INSERT INTO recipe_flavor (recipe_id, flavor_id, percentage) VALUES (?,?,?)"; 
